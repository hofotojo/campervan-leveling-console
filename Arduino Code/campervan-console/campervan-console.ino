 ...
 *  Github: https://github.com/hofotojo/campervan-leveling-console
 *  Includes:
 *  LCD display
...
void screen1(){
  ...
  int print_pitch = round(pitch - pitch_offset);
  int print_roll = round(roll - roll_offset);
 ...
 //Printing pitch
  ...
  lcd.setCursor(1,1);
  lcd.print("-");
  //Final pitch is inverted, as it is cleared in display this way which side to lift.
...
void screen4(){
  ...
  float vref = 0.0;
    ...
    while (sample_count < NUM_SAMPLES_VOLTAGE) {
    ...
    voltage = ((float)sum / (float)NUM_SAMPLES_VOLTAGE * 4.94) / 1024.0;
...
void longKeyPress() {
    ...
    while (i<CAL_LOOPS){
      readAcc();
      Serial.println(tp);
      tp = tp + pitch/CAL_LOOPS;
      tr = tr + roll/CAL_LOOPS;
      delay(200);
      i++;
    }    
    pitch_offset=tp;
